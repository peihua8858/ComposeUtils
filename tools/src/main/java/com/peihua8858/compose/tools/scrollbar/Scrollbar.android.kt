...
package com.peihua8858.compose.tools.scrollbar
...
fun defaultScrollbarStyle() = ScrollbarStyle(
    ...
    unhoverColor = Color(0x1a1F1F1F),
    hoverColor = Color(0x808F8F8F)
)
...
private fun Scrollbar(
    ...
    LaunchedEffect(adapter.scrollOffset) {
        ...
        hideJob.value = launch {
            delay(style.hideDelayMillis.toLong())
    ...
    val insetPx = style.scrollbarPadding.roundToPx()
    ...
    val measurePolicy = if (isVertical) {
        ...
        remember(sliderAdapter, scrollThickness, insetPx) {
            horizontalMeasurePolicy(sliderAdapter, { containerSize = it }, scrollThickness, insetPx)
        }
    }
    ...
    val targetAlpha by animateFloatAsState(
        targetValue = if ((isScrollbarVisible || isHighlighted) && isVisible) 1f else 0f,
        animationSpec = tween(durationMillis = style.hoverDurationMillis),
        label = "scrollbar_alpha"
    )

    // 动画偏移：隐藏时向右移动 (inset + thickness)
    ...
    Layout(
        {
            Box(
                Modifier
                    .offset {
                        if (isVertical) {
                            IntOffset(x = offsetDp.roundToPx(), y = 0)
                        } else {
                            IntOffset(x = 0, y = offsetDp.roundToPx())
                        }
                    }
                    .alpha(targetAlpha)
                    .background(
                        color = if (targetAlpha > 0.01f) color else Color.Transparent,
                        shape = style.shape
                    )
                    .scrollbarDrag(
...
private class SliderAdapter(
    ...
    /**
     * Actual position of a thumb within slider container, clamped to valid range.
     */
    var position: Float
        get() {
            val rawPos = if (reverseLayout) {
                containerSize - size - scrollPosition
            } else {
                scrollPosition
            }
            // 🔧 修复：确保 thumb 不超出容器边界
            return rawPos.coerceIn(0f, (containerSize - size).coerceAtLeast(0f))
        }
        set(value) {
