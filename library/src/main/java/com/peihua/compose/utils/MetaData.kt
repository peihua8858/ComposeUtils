...
fun applicationMetaDataOf(name: String): String? =
    ContextInitializer.context.appInfo.metaData.getString(name)

fun activityMetaDataOf(name: String): String? =
    ContextInitializer.context.activityInfo.metaData.getString(name)

 fun serviceMetaDataOf(name: String): String? =
    ContextInitializer.context.serviceInfo.metaData.getString(name)

inline fun <reified T : BroadcastReceiver> providerMetaDataOf(name: String): String? =
    ContextInitializer.context.providerInfo.metaData.getString(name)

inline fun <reified T : BroadcastReceiver> receiverMetaDataOf(name: String): String? =
    ContextInitializer.context.receiverInfo.metaData.getString(name)

inline fun <reified T> ComponentName() = ComponentName(ContextInitializer.context, T::class.java)